...
_model_for_get = KeyWordModelStrechyDocument(
    var_1='string',
    var_2=datetime.datetime(year=2000, month=1, day=1, hour=1, minute=1, second=1)
)
...
async def test_get_model(insert_model):
    _get_model = await KeyWordModelStrechyDocument.get(document_id=_model_for_get.id)
